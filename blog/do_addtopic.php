<?php
    ...
    if (($_POST['user_name']) || (!$_POST['topic_owner']) || (!$_POST['topic_title']) || (!$_POST['post_text'])) {
        header("Location: addtopic.php");
        exit;
    }
    //create safe values for input into the database
    $forum_id = $_GET['forum_id'];
    $clean_user_name = mysqli_real_escape_string($mysqli, $_POST['user_name']);
    ...
    $add_topic_sql = "INSERT INTO forum_topics(topic_title,forum_id, topic_create_time, topic_owner, user_name)
                        VALUES ('".$clean_topic_title ."', '".$forum_id."', now(), '".$clean_topic_owner."', '".$clean_user_name."')";
